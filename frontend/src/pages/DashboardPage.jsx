      ...
      <aside className="w-64 fixed h-full bg-[#0B0F19] border-r border-gray-800 hidden md:flex flex-col z-50 transition-all duration-300">
        ...
        <div className="mt-auto p-6 border-t border-gray-800/60">
          <button
            type="button"
            onClick={goProfile}
            className="w-full text-left rounded-2xl border border-indigo-500/15 bg-gradient-to-br from-indigo-950/40 via-[#111827]/90 to-[#0B0F19] p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.03)] transition-all duration-300 hover:border-indigo-500/35 hover:shadow-[0_12px_40px_-12px_rgba(79,70,229,0.45)] focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0B0F19] group"
          >
            <div className="flex items-center gap-3">
              <div className="relative shrink-0">
                <div className="absolute -inset-0.5 rounded-2xl bg-gradient-to-br from-indigo-400/50 to-violet-600/30 opacity-70 blur-[2px] group-hover:opacity-100 transition-opacity" aria-hidden />
                <div className="relative w-11 h-11 rounded-xl bg-gradient-to-br from-indigo-600/90 to-violet-700/80 flex items-center justify-center text-sm font-black text-white shadow-inner border border-white/10">
                  {(user.name?.trim()?.[0] || user.email?.trim()?.[0] || "G").toUpperCase()}
                </div>
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-[11px] font-black uppercase tracking-[0.12em] text-indigo-300/90 mb-0.5">Your profile</p>
                <p className="text-sm font-bold truncate text-white leading-tight">{user.name || "Gig Worker"}</p>
                <p className="text-[10px] text-slate-500 truncate mt-0.5">{user.platform || "Platform"} · {user.city || city}</p>
              </div>
              <ChevronRight size={18} className="shrink-0 text-slate-600 group-hover:text-indigo-400 transition-colors" aria-hidden />
            </div>
          </button>
        </div>
      </aside>
        ...
        <header className="h-16 flex items-center justify-between px-6 border-b border-gray-800 bg-[#0B0F19]/90 backdrop-blur-xl sticky top-0 z-40 transition-all">
          ...
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={goProfile}
              className="md:hidden p-2.5 text-gray-500 hover:text-indigo-400 hover:bg-indigo-500/5 rounded-xl transition-all duration-300"
